...
class ExtremaFinder(Block):
    ...
    def execute(self):
        ...
        if self.input['threshold'].type == 'scalar':
            thresholds = [self.input['threshold'].data] * self.input['image_series'].data.shape[0]
        else:
            thresholds = self.input['threshold'].data
...
class ThresholdMask(Block):
    ...
    def execute(self):
        ...
        if self.direction == 'bigger':
            out.data = out.data > self.input['threshold'].data
...
class Command(Block):
    ...
    def execute(self):
        out = self.input['image_series'].copy()
        out.data = self.command(out.data)
        self.sent_event(out)         
...
class Filter(Block):
    ...
    def execute(self):    
        ...
        if self.flat:
            print 'flattened'
            for image in self.input['image_series'].data:
        ...
        out = self.input['image_series'].copy()
        out.shape = shape
        out.data = np.vstack(filtered_images)
        if self.normalize:
            test = np.zeros((200, 200))
            test[100, 100] = 1
            test = self.filter(test)
            out.data /= test[100, 100] 
...
class CutOut(Block):
    ...
    def execute(self):
        view = self.input['image_series'].data
        image_cut = self.input['image_series'].copy()
        image_cut.data = view[self.range[0]:self.range[1]].copy()
...
class Mean(Block):
    ...
    def execute(self):
        print self.input['image_series'].data.shape
        averaged_im = [np.mean(im, 0) for im in np.vsplit(self.input['image_series'].data, self.parts)]
        out = self.input['image_series'].copy()
        out.data = np.vstack(averaged_im)
...
class spatialMean(Block):
    ...
    def execute(self):
        averaged_im = [np.mean(im, 1) for im in np.hsplit(self.input['image_series'].data, self.parts)]
        out = self.input['image_series'].copy()
        out.data = np.hstack(averaged_im)
...
class Add(Block):
    ...
    def execute(self):
        ...
        out = self.input['image_series'].copy()
        out.data = np.sum(self.input['image_series'].data.astype('float'), 0)
...
class RelativeChange(Block):
    ...
    def execute(self):    
        ...
        relative_change.data = ((self.input['image_series'].data - self.input['base_image'].data)
                           / self.input['base_image'].data)
...
class Prod(Block):
    ...
    def execute(self):    
        ...
        prod.data = (self.input['image_series'].data * 
                      self.input['image_series_2'].data)
...
class Change(Block):
    ...
    def execute(self):    
        ...
        change.data -= self.input['base_image'].data
        print change.shape
...
class MeanThreshold(Block):
    ...
    def execute(self):
        mean = np.mean(self.input['image_series'].data, 1)
        #std = np.std(self.input['image_series'].data, 1)
...
class UniformNoiser(Block):
    ...
    def execute(self):
        ...
        noised_series.data += np.random.uniform(high=self.strength,
                                                size=self.input['image_series'].data.shape)
...
class Splitter(Block):
    ...
    def execute(self):
        ...
        print num_pieces, self.input['image_series'].data.shape
        if self.hsplit:
            data = np.hsplit(self.input['image_series'].data, num_pieces)
        else:
            data = np.vsplit(self.input['image_series'].data, num_pieces)
...
class Reshape(Block):
    ...
    def execute(self):
        num_pieces = len(self.input['image_series'].name)                       
        datalength = len(self.input['image_series'].data)
        out = self.input['image_series'].copy()
        label_dim1 = out.label_dim1
        out.label_dim1 = out.name
        out.name = label_dim1
        out.data = out.data.reshape(-1, datalength / num_pieces).transpose()
        self.sent_event(out)
...
class Collector(Block):
    ...
    def execute(self): 
        self.buffer.append(self.input['image_series'].data.copy())
    ...
    def process_signal(self, signal_event):
        ...
        if signal_event.value == self.finish_signal:
            ...
            if self.finish_count == len(self.input):
                if self.hstack:
                    self.image_container.data = np.hstack(self.buffer)
                else:
                    self.image_container.data = np.vstack(self.buffer)
                self.buffer = []
...
class Activations(Block):
    ...
    def execute(self):
        ...
        for pos in zip(*np.where(positions)):
            ...
            activations.append(np.dot(self.input['image_series'].data, mask))
...
class PointMapping(Block):
    ...
    def execute(self):
        positions = zip(*np.where(self.input['image_series'].data)) 
...
class Response2DB(Block):
    ...
    def execute(self):
        ...
        latent1_pos = np.argmax(np.abs((self.input['latent'].data[:, 0])))
        latent1 = self.input['latent'].data[latent1_pos, 0]
        ...
        for ind in range(self.input['pca'].data.shape[1]):
            row = id_response + ind
            dict_glomID[row] = self.meas_id + '_' + str(ind)
            dict_Activation[row] = latent1 * self.input['pca'].data[0, ind]
            try:
...
class Response2Mx(Block):
    ...
    def execute(self):
        ...
        latent1_pos = np.argmax(np.abs((self.input['latent'].data[:, 0])))
        latent1 = self.input['latent'].data[latent1_pos, 0]
        activation = latent1 * self.input['pca'].data[0]
...
class VisPCA2(Block):
    ...
    def execute(self):
        ...
        for pc in range(self.numpcs):
            argnorm = np.argmax(np.abs(self.input['pcs'].data[:, pc]))
            norm.append(self.input['pcs'].data[argnorm, pc])
            p = ax_pc.plot(self.input['pcs'].data[:, pc] / norm[-1], color=colors[pc])
            if pc == 0:
                p[0].set_linewidth(3)
        ...
        for pc in range(self.numpcs):
            ...
            ax_handles[pc].plot(self.input['loadings'].data[pc] * norm[pc], 's-', color=colors[pc])
        ...
        for pc in range(self.numpcs):
            ax_handles[pc].plot([-0.5, len(self.input['loadings'].data[0]) + 0.5], [0, 0], 'k:')
            ...
            ax_handles[pc].set_xticks(range(0, len(self.input['loadings'].data[0])))
            ax_handles[pc].set_xlim([-0.5, len(self.input['loadings'].data[0]) + 0.5])
...
class VisPCA(Block):
    ...
    def execute(self):
        ...
        a = collections.BrokenBarHCollection.span_where(range(len(self.input['activations'].data)),
        ...
        for pc in range(self.numpcs):
            ax_pc.plot(self.input['pcs'].data[:, pc], color=colors[pc])
        ...
        for pc in range(self.numpcs):
            ...
            ax_handles[pc].plot(np.arange(0.5, num_odors + 0.5), self.input['loadings'].data[pc], 's-', color=colors[pc])
...
class Visualize(Block):
    ...
    def execute(self):
        no_images = self.input['image_series'].data.shape[0]
        ...
        if not(self.color_scale == 'ind'):
            if self.color_scale:
                ...
                vmax = np.max(self.input['image_series'].data)
                vmin = np.min(self.input['image_series'].data)
            ...
            im = plt.imshow(np.zeros(self.input['image_series'].data.shape),
...
class PCAProjection(Block):
    ...
    def execute(self):
        ...
        self.pca.fit(self.input['image_series'].data)
        out = self.input['image_series'].copy()
        #back and forth projection
        out.data = np.dot(np.dot(out.data, self.pca.components_.T), self.pca.components_)
...
class PCA(Block):
    ...
    def execute(self):
        ...
        out.time = self.pca.transform(self.input['image_series'].data)
        out.data = out.base
        out.eigen = self.pca.explained_variance_ratio_
        self.sent_event(out)
    ...
    '''            
    def execute(self):
        image = self.input['image_series'].data
...
class sICA(Block):
    ...
    def execute(self):
        ...
        if self.latent_series:
            ...
            print 'imag'
            base = self.pca.fit_transform(self.input['image_series'].data.T)
            base2 = np.dot(self.pca.components_, self.input['image_series'].data)
...
class ICA(Block):
    ...
    def execute(self):
        '''
        data = self.input['image_series'].data
        ...
        #print self.input['image_series'].data.T.shape
        self.ica.fit(self.input['image_series'].data)
        projmatrix = self.ica.unmixing_matrix_
        print projmatrix.shape
                #latent = np.dot(data, fcanode.get_projmatrix())
        #projection = np.dot(self.input['projection'], fcanode.get_projmatrix())
        out = self.input['image_series'].copy()
        #out.data = fcanode.get_projmatrix()
        out.data = projmatrix
        self.sent_event(out)
...
class NNMA(Block):
    ...
    def execute(self):
        self.data = self.input['image_series'].data
        self.A, self.X, self.obj, count, converged = self.routine(self.input['image_series'].data,
    ...
    def group(self, threshold=0.05):
        self.A, self.X = self.routine.group_time(self.data, self.A, self.X, threshold)
    
    def degroup(self, threshold=0.4, justshow=False):
        self.A, self.X = self.routine.degroup_base(self.data, self.A, self.X, threshold, justshow=justshow)   
...
class Projection(Block):
    ...
    def execute(self):
        out = self.input['image_series'].copy()
        print self.input['image_series'].data.shape, self.input['base'].data.shape
        if self.dim == 0:
            out.data = np.dot(out.data, self.input['base'].data[:self.components].T)
        elif self.dim == 1:
            out.data = np.dot(out.data, self.input['base'].data[:self.components])
        print 'projection output shape: ', out.data.shape
        self.sent_event(out)
...
class Align(Block):
    ...
    def execute(self):
        ...
        for image in self.input['image_series'].shaped2D():
            ...
            y.append(yt)
        out = self.input['image_series'].copy()
        out.data = np.array([x, y]).transpose()
        self.sent_event(out)
